...
class ConfigsCalculateParams(BaseModel):
    ...
    def check_attrs_are_types(cls, model):
        """_summary_"""
        attr_ls = model.model_extra.keys()
        return PydanticModelMixin.validate_attrs(model, attr_ls, ConfigsUnitProcess)
...
class ConfigsEvalVid(BaseModel):
    ...
    def validate_cmap(cls, v):
        """_summary_"""
        return PydanticModelMixin.validate_attr_closed_set(v, plt.colormaps())

    @field_validator("colour_level")
    @classmethod
    def validate_colour_level(cls, v):
        """_summary_"""
        return PydanticModelMixin.validate_attr_closed_set(v, DLC_COLUMN_NAMES)
